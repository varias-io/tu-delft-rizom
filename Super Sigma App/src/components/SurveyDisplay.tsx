  ...
  return <>
    {await Promise.all(surveys.map(async (survey) => {
      ...
      return <>
        ...
        <Actions>
          {displayedInModal || personalProgress==15 ? <></> : <Button style="primary" actionId="fillSurvey" value={JSON.stringify({surveyId: survey.id, channelId: undefined})}>Fill in Survey</Button>}
